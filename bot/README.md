...
| `profile.py` | загрузка профиля (ниша: критерий, категории, источники) |
| `profiles/<name>/` | **конфиг конкретного бота — правьте его под себя** |
| `collector.py` | сбор из RSS, Telegram (веб-превью), YouTube |
| `llm.py` | Gemini / Groq (перебор моделей + ретраи + fallback) |
| `formatter.py` | вёрстка дайджеста |
| `telegram_sender.py` | отправка в Telegram |
| `state.py` | память между запусками (`profiles/<name>/state.json`) |

### 🧩 Профили (несколько ботов на одном коде)

Каждый тематический бот — это папка `profiles/<name>/`:

- `profile.yaml` — тема: заголовок, критерий отбора для LLM, категории, настройки классики;
- `sources.yaml` — **источники (RSS / Telegram / YouTube) — правьте под себя**;
- `evergreen.json` — библиотека «вечнозелёных» инсайтов (если включена);
- `state.json` — память бота (антидубли), коммитится автоматически.

Профиль выбирается переменной `BOT_PROFILE` (в воркфлоу). Сейчас есть два:
`edtech` (маркетинг EdTech) и `gamedev` (бизнес игровой индустрии). Каждому —
свой воркфлоу и свой телеграм-бот (свои секреты токена/чата).
...
#### Второй бот — GameDev (профиль `gamedev`)

Работает на том же коде и тех же LLM-ключах, но через **отдельного
телеграм-бота**. Создайте нового бота у [@BotFather](https://t.me/BotFather)
и добавьте ещё два секрета:

| Secret | Значение |
|---|---|
| `TELEGRAM_BOT_TOKEN_GAMEDEV` | токен нового бота |
| `TELEGRAM_CHAT_ID_GAMEDEV` | id получателя (ваш chat_id или канал) |

Воркфлоу `GameDev Biz Digest` (`.github/workflows/digest-gamedev.yml`)
запускается отдельно, в те же 10:00 по Варшаве.
